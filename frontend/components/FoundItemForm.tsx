...
import { MapPin, Calendar, Upload, X, Camera } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";
import { itemsService } from "@/utils/itemsService";
import { Category, FoundItemForm as FoundItemFormData } from "@/types/LostFound";
...
export function FoundItemForm({ onSuccess }: FoundItemFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await itemsService.createFoundItem(formData, images);
      toast({
        title: "Item Reported",
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to report found item",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
        ...
        <ul className="text-sm text-blue-700 space-y-1">
          <li>• Only add clear, high-quality photos of the item</li>
          <li>• Be cautious about sharing personal information publicly</li>
          <li>• Consider meeting in public places when returning items</li>
